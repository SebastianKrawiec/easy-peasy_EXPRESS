...
let listaDePeliculas = function(){
    let lista = movies.map((movie)=>{
		return `\nNombre: ${movie.title}`;
    });
    return lista;
}

let listaDeReseñas = function(){
    let Reseñas = movies.map((movie)=>{
        return `Nombre: ${movie.title}\nReseña: ${movie.overview}\n\n`
    });
    return Reseñas;
}

let puntuadas = function(){
	let listaPuntuadas = movies.filter((movie) =>{
		if(movie.vote_average >= 7){
			return movie.title;
		}
	});
	listaPuntuadas.forEach((movie) => {
		return `\nNombre : ${movie.title} || Votos: ${movie.vote_average}\n\nReseña: ${movie.overview}`;
	});
	return listaPuntuadas;
};

let ordenAlfabetico = function(arr){
    return arr.sort();
};

let promedio = function(){
	let num = movies.map((movie) =>{
		return movie.vote_average;
	})
	let cantidad = num.length;
	let promedio = num.reduce((acumulador,valor) => {
...
let salas = function(){
	let salas = theaters.map((theater) => {
		return `Nombre: ${theater.name}\nDireccion:\n${theater.address}\nDescripcion:\n${theater.description}\n\n`
	});
	return salas;
}

let preguntas = function(){
	let pre = faqs.map((pregunta) =>{
		return `P: ${pregunta.faq_title}\n R: ${pregunta.faq_answer}\n\n`;
	})
	return pre;
}
	...
	switch (req.url) {
		...
		case '/':
            ...
            res.write(`Bienvenidos a DH Movies el mejor sitio para encontrar las mejores películas, incluso mucho mejor  que Netflix, Cuevana y PopCorn​.
                       ...
                       \nPeliculas por orden Alfabetico: \n${ordenAlfabetico(listaDePeliculas())}`);
            res.end(`\n\n\n\n\n\nRecordá que podés visitar las secciones:\nEn Cartelera, Más Votadas, Sucursales, Contacto y Preguntas Frecuentes\n\nEstas en: Home`);
			break;
		// En cartelera
		case '/en-cartelera':
            res.write(`Cartelera\n\nTotal de Peliculas en Cartelera: ${movies.length}\n\nListado de Peliculas:\n\n${listaDeReseñas()}`)
			res.end(`\n\n\n\n\n\nRecordá que podés visitar las secciones:\nMás Votadas, Sucursales, Contacto y Preguntas Frecuentes`);
            break;
        // Mas Votadas   
		case '/mas-votadas':
            res.write(`Más votadas\n\nTotal de Peliculas: ${puntuadas().length}\n\nRating promedio: ${promedio()}\n\nLista de Peliculas:\n${puntuadas()}`);
			res.end(`\n\n\n\n\n\nRecordá que podés visitar las secciones:\nEn Cartelera, Sucursales, Contacto y Preguntas Frecuentes`);
            break;
        // Sucursales
		case '/sucursales':
			res.write(`Nuestras Salas\n\nTotal de salas:\n${theaters.length}\n\nListado de salas:\n\n${salas()}`);
			res.end(`\n\n\n\n\n\nRecordá que podés visitar las secciones:\nEn Cartelera, Más Votadas, Contacto y Preguntas Frecuentes`);
            break;
        // Contacto
		case '/contacto':
		...
		case '/preguntas-frecuentes':
			...
			res.end(`\n\n\n\n\n\nRecordá que podés visitar las secciones:\nEn Cartelera, Más Votadas, Sucursales y Contacto`);
			break;
		default:
